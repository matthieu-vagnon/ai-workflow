...
export async function runManage({ TOOLS, CATEGORIES, INTERMEDIATE_DIR, STABLE_CONFIG_DIR }) {
  ...
  if (toolsToAdd.length > 0) {
    p.log.success(
      `Tools (add-only): added ${toolsToAdd.map((t) => t.label).join(", ")}`,
    );
  } else {
    p.log.info("Tools (add-only): no changes");
  }
  ...
  for (const category of CATEGORIES) {
    ...
    const { projectSensitive, generic, currentGeneric, currentProjectSensitive } =
      scanCurrentState(category, intermediateBase, STABLE_CONFIG_DIR);
    ...
    if (availableProjectSensitive.length === 0 && availableGeneric.length === 0) {
      p.log.info(`${capitalize(category)} (add-only): no changes`);
      continue;
    }
    ...
    p.log.success(`${capitalize(category)} (add-only): ${changes.join(" | ")}`);
  }

  p.outro("Done (add-only)");
}
