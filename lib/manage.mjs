...
export async function runManage({
  ...
  for (const category of CATEGORIES) {
    ...
    for (const { name, dep } of depSensitive.filter(
      (d) => !currentDepSensitive.some((c) => c.name === d.name && c.dep === d.dep),
    )) {
      allOptions.push({
  ...
  if (allOptions.length === 0) {
    ...
    for (const val of selected || []) {
      ...
      if (type === "ps") toAdd[category].projectSensitive.push(rest);
      else if (type === "dep") {
        const lastSlash = rest.lastIndexOf("/");
        const dep = rest.substring(0, lastSlash);
        const name = rest.substring(lastSlash + 1);
        toAdd[category].depSensitive.push({ name, dep });
      } else toAdd[category].generic.push(rest);
    }
    ...
    for (const category of CATEGORIES) {
      ...
      for (const { name, dep } of items.depSensitive) {
        ...
        copyPath(srcPath, intermediatePath);
        manifest.items[category][name] = `dep-sensitive:${dep}`;
      ...
      if (items.depSensitive.length)
        changes.push(`dep-sensitive: ${items.depSensitive.map((d) => d.name).join(", ")}`);
...
function addTool(
  ...
  // Config files (copy template as-is + copy to .mvagnon-agents/ as example)
  for (const [src, dest] of Object.entries(tool.configFiles)) {
    ...
    if (fs.existsSync(srcPath)) {
      ...
      // Also copy template to intermediate dir as example
      const examplePath = path.join(intermediateBase, src);
...
function scanCurrentState(category, intermediateBase, configDir) {
  ...
  // dep-sensitive: nested structure {dep}/{skill}
  const depDir = path.join(configDir, category, "dep-sensitive");
  ...
  for (const [item, type] of Object.entries(manifestItems)) {
    ...
    else if (type.startsWith("dep-sensitive:")) {
      const dep = type.slice("dep-sensitive:".length);
      currentDepSensitive.push({ name: item, dep });
    } else if (type === "dep-sensitive") currentDepSensitive.push({ name: item, dep: "" });
  }
