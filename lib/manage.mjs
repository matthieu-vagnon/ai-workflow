...
export async function runManage({ TOOLS, CATEGORIES, INTERMEDIATE_DIR, STABLE_CONFIG_DIR }) {
  ...
  for (const tool of toolsToAdd) {
    addTool(tool, projectRoot, intermediateBase, STABLE_CONFIG_DIR, CATEGORIES, useExceptions);
  }
  ...
  for (const category of CATEGORIES) {
    ...
    let selectedProjectSensitive = currentProjectSensitive;
    if (projectSensitive.length > 0) {
      const psOptions = projectSensitive.map((name) => ({
        value: name,
        label: name,
        initialSelected: currentProjectSensitive.includes(name),
      }));

      selectedProjectSensitive = await p.multiselect({
        message: `Select project-sensitive ${category}`,
        options: psOptions,
        required: false,
      });
    ...
    let selectedGeneric = currentGeneric;
    if (generic.length > 0) {
      ...
      selectedGeneric = await p.multiselect({
    ...
    const selectedProjectSet = new Set(selectedProjectSensitive || []);
    const currentProjectSet = new Set(currentProjectSensitive);
    const projectToAdd = [...selectedProjectSet].filter(
      (x) => !currentProjectSet.has(x),
    );
    const projectToRemove = [...currentProjectSet].filter(
      (x) => !selectedProjectSet.has(x),
    );

    const selectedGenericSet = new Set(selectedGeneric || []);
    const currentGenericSet = new Set(currentGeneric);
    const genericToAdd = [...selectedGenericSet].filter(
      (x) => !currentGenericSet.has(x),
    );
    const genericToRemove = [...currentGenericSet].filter(
      (x) => !selectedGenericSet.has(x),
    );

    if (
      projectToAdd.length === 0 &&
      projectToRemove.length === 0 &&
      genericToAdd.length === 0 &&
      genericToRemove.length === 0
    ) {
    ...
    // Apply project-sensitive removals
    for (const item of projectToRemove) {
      const intermediatePath = path.join(psIntermediateDir, item);
    ...
    // Apply generic removals
    for (const item of genericToRemove) {
      const intermediatePath = path.join(genIntermediateDir, item);
      removePath(intermediatePath);
    ...
    if (projectToRemove.length)
      changes.push(`project-sensitive removed: ${projectToRemove.join(", ")}`);
    if (genericToAdd.length)
      changes.push(`generic added: ${genericToAdd.join(", ")}`);
    if (genericToRemove.length)
      changes.push(`generic removed: ${genericToRemove.join(", ")}`);
...
function addTool(tool, projectRoot, intermediateBase, stableConfigDir, categories, useExceptions = false) {
