...
export async function runManage({
  ...
  // --- Resource management (rules, skills in one menu) ---
  ...
  if (allOptions.length === 0) {
    ...
    for (const val of selected || []) {
      ...
      if (!toAdd[category])
        toAdd[category] = { projectSensitive: [], generic: [] };
    ...
    for (const category of CATEGORIES) {
      ...
      if (
        !items ||
        (items.projectSensitive.length === 0 && items.generic.length === 0)
      )
        continue;
      ...
      for (const item of items.projectSensitive) {
      ...
      const genIntermediateDir = path.join(
        intermediateBase,
        "generic",
        category,
      );

      for (const item of items.generic) {
