...
1. Call `mcp__exa__search` with a focused query (3-6 words, include library name + version if relevant);
2. Call `mcp__exa__get-contents` on promising results to get full page content;
3. Prefer official sources (docs sites, GitHub repos, RFCs) over blog posts.

## Rules

- NEVER skip Step 1 and go directly to Exa;
